...
MODEL_PATH = "fracture_model_v2.keras"
FILE_ID    = "13MwFHWVqzr0Q4QdERd9PTp8jk_Oiyy91"

if not os.path.exists(MODEL_PATH):
    with st.spinner("⏬ Downloading model... (first launch only, ~208MB)"):
...
<div style="text-align:center; padding:3rem 0 1.5rem;">
    <div style="display:inline-block; background:linear-gradient(135deg,rgba(0,212,255,0.12),rgba(123,97,255,0.12));
                border:1px solid rgba(0,212,255,0.25); border-radius:50px; padding:0.4rem 1.2rem;
                font-family:'DM Sans',sans-serif; font-size:0.78rem; letter-spacing:0.15em;
                color:#00d4ff; text-transform:uppercase; margin-bottom:1rem;">
        AI-Powered Radiology Assistant
    </div>
    <h1 style="font-family:'Syne',sans-serif; font-size:clamp(2.8rem,6vw,5rem); font-weight:800;
               letter-spacing:-0.02em; line-height:1;
               background:linear-gradient(135deg,#ffffff 0%,#00d4ff 50%,#7b61ff 100%);
               -webkit-background-clip:text; -webkit-text-fill-color:transparent;
               background-clip:text; margin-bottom:0.6rem;">OsteoScan AI</h1>
    <p style="font-family:'DM Sans',sans-serif; font-size:1.1rem; color:#6b7280;
              font-weight:300; max-width:520px; margin:0 auto 0.5rem;">
        Deep learning bone fracture detection with<br>Grad-CAM explainability — 96% accuracy
    </p>
</div>
""", unsafe_allow_html=True)
...
<div style="display:flex; justify-content:center; gap:1rem; flex-wrap:wrap; margin-bottom:2.5rem;">
    <div style="background:rgba(0,212,255,0.08); border:1px solid rgba(0,212,255,0.2);
                border-radius:50px; padding:0.45rem 1.2rem; font-size:0.85rem; color:#00d4ff;">
        ⚡ 96% Accuracy
    </div>
    <div style="background:rgba(123,97,255,0.08); border:1px solid rgba(123,97,255,0.2);
                border-radius:50px; padding:0.45rem 1.2rem; font-size:0.85rem; color:#7b61ff;">
        🧠 EfficientNetB4 + Grad-CAM
    </div>
    <div style="background:rgba(46,213,115,0.08); border:1px solid rgba(46,213,115,0.2);
                border-radius:50px; padding:0.45rem 1.2rem; font-size:0.85rem; color:#2ed573;">
...
if uploaded_file:
    ...
    if analyze:
        ...
        status.markdown("<p style='color:#00d4ff;font-size:0.9rem;'>⚙️ Preprocessing with CLAHE...</p>", unsafe_allow_html=True)
        ...
        status.markdown("<p style='color:#00d4ff;font-size:0.9rem;'>🧠 Running EfficientNetB4 inference...</p>", unsafe_allow_html=True)
        for i in range(30, 65): prog.progress(i); time.sleep(0.01)

        pred_prob = float(model(img_input, training=False)[0][0])

        status.markdown("<p style='color:#00d4ff;font-size:0.9rem;'>🔥 Generating Grad-CAM heatmap...</p>", unsafe_allow_html=True)
        ...
        <div style="background:{rb}; border:1px solid {rbr}; border-radius:20px;
            ...
            <div>
                <div style="font-family:'DM Sans',sans-serif; font-size:0.75rem; letter-spacing:0.15em;
                            text-transform:uppercase; color:{rc}; margin-bottom:0.3rem;">Diagnosis Result</div>
                <div style="font-family:'Syne',sans-serif; font-size:2rem; font-weight:800; color:{rc};">
                    {result_icon} {pred_label}
                </div>
                <div style="font-family:'DM Sans',sans-serif; font-size:0.9rem; color:#9ca3af;
                            margin-top:0.4rem; max-width:480px;">{recommendation}</div>
            </div>
        ...
        for col, label, value, color in [
            ...
            with col:
                st.markdown(f"""
                <div style="background:#161a22; border:1px solid #1f2535; border-top:3px solid {color};
        ...
        <div style="background:rgba(123,97,255,0.07); border:1px solid rgba(123,97,255,0.2);
                    border-left:4px solid #7b61ff; border-radius:12px; padding:1.2rem 1.5rem; margin:2rem 0 0.5rem;">
